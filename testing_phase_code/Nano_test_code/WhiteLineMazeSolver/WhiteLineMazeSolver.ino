...
const unsigned long LONG_PRESS_DELAY = 100; // Minimum press duration
...
#define MaxSpeed 210 //170
#define BaseSpeed 160 //170
int lastError = 0;
float kp = 0.5;    // It fully depends on the bot system
float kd = 10;    // Please follow the method provided in instructables to get your values
int last_pos = 3500;
...
void setup()
{
  ...
  // }
 // Wait for s1 button press with debouncing
s1 = digitalRead(sw1);
while (s1 == HIGH)
{
  s1 = digitalRead(sw1);
...
while (millis() - pressStart < DEBOUNCE_DELAY)
{
  if (digitalRead(sw1) == HIGH) // Button released too quickly (bounce)
  {
    // Wait for button to be pressed again
    while (digitalRead(sw1) == HIGH);
    pressStart = millis(); // Reset timer
  }
}

// Wait for s1 release with debouncing
while (digitalRead(sw1) == LOW);
delay(DEBOUNCE_DELAY); // Debounce the release

digitalWrite(led, HIGH);
delay(800);
calibration();

// Rule selection with debouncing
while (1)
{
  ...
  if (s2 == LOW)
  {
    ...
    if (digitalRead(sw2) == LOW) // Confirm button still pressed
    {
      // Wait for minimum press duration
      unsigned long pressTime = millis();
      while (digitalRead(sw2) == LOW && (millis() - pressTime) < LONG_PRESS_DELAY);
      
      if (millis() - pressTime >= LONG_PRESS_DELAY) // Valid press
      {
        chr = 1;
        ...
        break; // LHS: left, straight, right priority
      }
    }
  }
  ...
  {
    delay(DEBOUNCE_DELAY); // Debounce delay
    if (digitalRead(sw3) == LOW) // Confirm button still pressed
    {
      // Wait for minimum press duration
      unsigned long pressTime = millis();
      while (digitalRead(sw3) == LOW && (millis() - pressTime) < LONG_PRESS_DELAY);
      ...
      if (millis() - pressTime >= LONG_PRESS_DELAY) // Valid press
      {
        ...
        break; // RHS: right, straight, left priority
      }
    }
  }
}
...
// Print minimum calibration values
...
Serial.println();  // New line after array

// Print maximum calibration values
Serial.println("\nMaximum Array:");
for (int i = 0; i < NUM_SENSORS; i++) 
{
